...
public class IncludeHtmlTagTest {
    ...
    @Test
    public void getPassedVariablesWithQuotes() {
        String htmlString = "<meta ht-include=\"path/to/file\" ht-variables=\"name,@name.substring(2,@name.indexOf(',hgh')),maps,(Map<String,String>)maps\" >";
        Template template = Mockito.mock(Template.class);
        IncludeHtmlTag includeHtmlTag = new RuntimeIncludeHtmlTag(htmlString, Code::parseForFunction, template);
        Map<String, String> passedVariables = includeHtmlTag.getPassedVariables();
        Map<String, String> expectedPassedVariables = Map.of(
                "name", "name().substring(2,name().indexOf(\",hgh\"))",
                "maps", "(Map<String,String>)maps"
        );
    ...
    @Test
    public void getPassedVariablesWithStringContainingComma() {
        String htmlString = "<meta ht-include=\"path/to/file\" ht-variables=\"name,'a,b',age,12\" >";
        Template template = Mockito.mock(Template.class);
        IncludeHtmlTag includeHtmlTag = new RuntimeIncludeHtmlTag(htmlString, Code::parseForFunction, template);
        Map<String, String> passedVariables = includeHtmlTag.getPassedVariables();
        Map<String, String> expectedPassedVariables = Map.of(
